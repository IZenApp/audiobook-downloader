...
show_header() {
    ...
    echo -e "${PURPLE}║${CYAN}                     Версия 2.0 - Полная                      ${PURPLE}║${NC}"
    echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
show_menu() {
    ...
    echo -e "${GREEN}3.${NC} ${WHITE}Простая версия v3${NC} ${CYAN}(MP3, короткие имена файлов)${NC}"
    echo -e "${GREEN}4.${NC} ${WHITE}Улучшенная версия${NC} ${CYAN}(Google + YouTube поиск)${NC}"
    echo ""
    echo -e "${BLUE}5.${NC} ${WHITE}Статистика${NC} ${CYAN}(Анализ скачанных файлов)${NC}"
    echo -e "${BLUE}6.${NC} ${WHITE}Тесты${NC} ${CYAN}(Проверка работоспособности)${NC}"
    echo ""
    echo -e "${YELLOW}7.${NC} ${WHITE}Помощь${NC} ${CYAN}(Документация)${NC}"
    echo -e "${RED}8.${NC} ${WHITE}Выход${NC}"
    echo ""
    echo -ne "${YELLOW}Ваш выбор [1-8]: ${NC}"
}
...
while true; do
    ...
    case $choice in
        ...
        3)
            ...
            ;;
        4)
            run_command "python src/utils/robust_downloader.py" "Улучшенная версия"
            ;;
        5)
            run_command "python src/utils/stats.py" "Статистика"
            ;;
        6)
            run_command "python tests/test_core.py" "Тесты"
            ;;
        7)
            show_header
            ...
            echo -e "${WHITE}• docs/FIXES.md${NC} - История исправлений"
            echo ""
        ...
        8)
            ...
            ;;
        *)
            echo -e "${RED}❌ Неверный выбор. Пожалуйста, введите число от 1 до 8.${NC}"
            echo -e "${YELLOW}Нажмите Enter для продолжения...${NC}"
            read
            ;;
    esac
